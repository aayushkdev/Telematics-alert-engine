import pytest
from pydantic import ValidationError

from app.schemas.rule import RuleCreate, RuleUpdate


def valid_rule_data() -> dict[str, object]:
    return {
        "organization_id": 1,
        "name": "Speeding",
        "rule_type": "simple",
        "field": "speed_mph",
        "operator": ">",
        "threshold": 70,
    }


def rule_data(**changes: object) -> dict[str, object]:
    data = valid_rule_data()
    data.update(changes)
    return data


def test_simple_rule_is_valid() -> None:
    rule = RuleCreate(**valid_rule_data())
    assert rule.rule_type.value == "simple"


@pytest.mark.parametrize("field", ["engine_state", "location", "unknown"])
def test_disallowed_field_is_rejected(field: str) -> None:
    with pytest.raises(ValidationError, match="field must be one of"):
        RuleCreate(**rule_data(field=field))


@pytest.mark.parametrize("operator", ["!=", "contains", "and"])
def test_disallowed_operator_is_rejected(operator: str) -> None:
    with pytest.raises(ValidationError, match="operator must be one of"):
        RuleCreate(**rule_data(operator=operator))


def test_windowed_rule_is_rejected() -> None:
    with pytest.raises(ValidationError, match="only simple rules"):
        RuleCreate(**rule_data(rule_type="windowed"))


@pytest.mark.parametrize("field", ["window_seconds", "min_matching_events"])
def test_simple_rule_rejects_window_settings(field: str) -> None:
    with pytest.raises(ValidationError, match="simple rules cannot include window settings"):
        RuleCreate(**rule_data(**{field: 60}))


@pytest.mark.parametrize("field", ["suppress_for_seconds", "escalate_after_seconds"])
def test_negative_duration_is_rejected(field: str) -> None:
    with pytest.raises(ValidationError, match="duration must be zero or positive"):
        RuleCreate(**rule_data(**{field: -1}))


def test_windowed_rule_update_is_rejected() -> None:
    with pytest.raises(ValidationError, match="only simple rules"):
        RuleUpdate(rule_type="windowed")
