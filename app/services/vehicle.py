...
from app.models import Vehicle, Driver, Organization
from app.schemas.vehicle import VehicleCreate, VehicleUpdate, VehicleAssignDriver
...
async def get_by_id(db: AsyncSession, vehicle_id: int, organization_id: int) -> Vehicle | None:
...
async def update(db: AsyncSession, vehicle_id: int, organization_id: int, data: VehicleUpdate) -> Vehicle | None:
...
async def assign_driver(
    ...
    if data.driver_id is not None:
        result = await db.execute(
            select(Driver).where(Driver.id == data.driver_id, Driver.organization_id == organization_id)
        )
        if not result.scalar_one_or_none():
            return None
